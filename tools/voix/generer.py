...
# Plusieurs générateurs en parallèle : WORKER=i/n traite une réplique sur n.
WI, WN = map(int, os.environ.get('WORKER', '0/1').split('/'))
tts = ChatterboxMultilingualTTS.from_pretrained(device='cpu')
# Accélération (mesurée) : 6 pas de diffusion au lieu de 10 pour la
...
for n, j in enumerate(jobs):
    ...
    if n % WN != WI: continue
    if REDO and j['id'] not in REDO: continue
    if os.path.exists(dst) and not REDO: continue
    ...
    for take in range(TAKES):
        ...
        if score >= (0.85 if REDO else 0.75): break
    if REDO and best[0] <= OLD.get(j['id'], -9):
        print(n, j['id'], 'garde l\'ancienne', round(best[0], 2), flush=True); continue
    p = v['pitch']
